...
def missingfields(dict_, obj):
    ...
    if missing:
        logtext = "Missing fields in package " + '???' + ": " + str(obj.__name__) + " " + str(missing) + "\n"
        log(logtext)
    for m in missing:
        del dict_[m]
...
def parse(string,verbose=False):
    doc = etree.fromstring(string)
    activities = []
    for x in doc.findall("iati-activity"):
        activity = parse_activity(x)
